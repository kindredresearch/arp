...
def traj_segment_generator(pi, env, horizon, stochastic, ar):
    ...
    past_x_next = np.zeros((ar.p, ac.shape[-1]))    # past p values of an AR process X_t
...
def learn(env, policy_fn, *,
    ...
    ratio = tf.exp(tf.clip_by_value(pi.pd.logp(ac, past_x, update_mask) - oldpi.pd.logp(ac, past_x, update_mask), -np.inf, 5)) # pnew / pold
    ...
    compute_losses = U.function([ob, ac, atarg, ret, lrmult, past_x, update_mask], losses)
    ...
    while True:
        ...
        d = Dataset(dict(ob=ob, ac=ac, atarg=atarg, vtarg=tdlamret, past_x=past_x, update_mask=update_mask), shuffle=not pi.recurrent)
        ...
        losses = []
        for batch in d.iterate_once(optim_batchsize):
            newlosses = compute_losses(batch["ob"], batch["ac"], batch["atarg"], batch["vtarg"], cur_lrmult, batch['past_x'], batch['update_mask'])
            losses.append(newlosses)
